...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM exam_attempts WHERE user_id = ? AND is_completed = 1");
$stmt->execute([$user['id']]);
$total_attempts = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT AVG(total_score) FROM exam_attempts WHERE user_id = ? AND is_completed = 1");
$stmt->execute([$user['id']]);
...
$stmt = $pdo->prepare("SELECT MAX(total_score) FROM exam_attempts WHERE user_id = ? AND is_completed = 1");
$stmt->execute([$user['id']]);
...
$stmt = $pdo->prepare("
    ...
    WHERE ea.user_id = ? AND ea.is_completed = 1
